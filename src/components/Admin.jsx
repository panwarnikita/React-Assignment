...
import { Mail, Shield, User, GraduationCap, Search, FileText } from 'lucide-react';
    ...
    <div className="max-w-6xl mx-auto font-sans">
      ...
      <div className="mb-8 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 mt-1 text-sm">Manage access, roles, and track student progress.</p>
        </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
                        ...
                        <div className="flex flex-wrap gap-1.5">
                          {user.enrolledCourses && user.enrolledCourses.length > 0 
                            ? user.enrolledCourses.map(cId => (
                                <span key={cId} className="bg-slate-100 text-slate-600 px-2 py-0.5 rounded text-[11px] font-medium border border-gray-200 flex items-center gap-1">
                                  <FileText size={10} /> ID: {cId}
                                </span>
                              ))
        ...
        <div className="bg-gray-50/50 px-6 py-4 border-t border-gray-200 flex items-center justify-between">
           <span className="text-xs text-slate-500">Showing {filteredUsers.length} results</span>
           <div className="flex gap-2">
             <button className="px-3 py-1 text-xs font-medium text-slate-500 bg-white border border-gray-200 rounded hover:bg-gray-50 disabled:opacity-50" disabled>Previous</button>
             <button className="px-3 py-1 text-xs font-medium text-slate-500 bg-white border border-gray-200 rounded hover:bg-gray-50 disabled:opacity-50" disabled>Next</button>
           </div>
        </div>
      </div>
    </div>
